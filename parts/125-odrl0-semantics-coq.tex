...
The \syn{primPolicySet} which was passed to \syn{process_single_ps} was either constructed by a \syn{PIPS} constructor or by a \syn{PEPS} constructor, which distinguish between \syn{primInclusivePolicySet} and \syn{primExclusivePolicySet} types. The translation functions \syn{trans_policy_PIPS} (see listing~\ref{lst:transpipsCoq}) and \syn{trans_policy_PEPS} (see listing~\ref{lst:transpepsCoq}) are called in turn as the case may be.
