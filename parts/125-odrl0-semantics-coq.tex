...
\syn{sumbool} is a boolean type defined in the Coq standard library module \syn{Coq.Init.Specif}. \syn{sumbool} captures the idea of program values that indicate which of two propositions is true~\cite{chlipalacpdt2011}. \syn{sumbool} is a boolean type equipped with the justification of their value, which really helps with proofs. \syn{sumbool A B}, which is written \syn{\{A\}+\{B\}}, is the informative disjunction ``A or B'', where A and B are logical propositions. Its extraction is isomorphic to the type of booleans. A boolean is either true or false, and this is decidable~\cite{Coq:manual}. See listing~\ref{lst:sumbooltypeCoq} for the definition of \syn{sumbool} and \syn{sumbool_of_bool}; note the notational convenience where curly braces around each proposition along with the addition symbol are used to represent the \syn{sumbool} type.
