...
In the following paragraphs, we will describe each translation function in detail however a high-level description of how the main algorithm (encoded in these translation functions) works is in order.

\begin{algorithm}      
\caption{For an inclusive policy set, given a query, calculate a result with a decision of Permitted, NotPermitted, or Unregulated}       
\label{inclusivePS}   
\begin{algorithmic}        
...
\begin{algorithm}      
\caption{For an exclusive policy set, given a query, calculate a result with a decision of Permitted, NotPermitted, or Unregulated}       
\label{exclusivePS}   
\begin{algorithmic}        
    \IF{$asset\_from\_query = asset\_from\_agreement$}
      \IF{$subject\_from\_query$ is in $prin\_u$}
