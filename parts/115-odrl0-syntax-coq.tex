...
A \syn{policy} is defined as a datatype with the constructors \syn{Policy} which takes a nonempty list of primitive policies, or \syn{primPolicy}s. A \syn{primPolicy} is constructed by calling $\linebreak$ \syn{PrimitivePolicy} which takes a \syn{preRequisite}, a \syn{policyId} and an action \syn{act} (see listing~\ref{lst:primPolicyast} for the abstract syntax version). Ignoring the \syn{policyId} for a moment, a primitive policy consists of a prerequisite and an action. If the prerequisite holds, the action is allowed to be performed on the asset. 
...
Finally a \syn{constraint} (see  listing~\ref{lst:constraintast} for the abstract syntax equivalent) is defined as a new datatype with constructors \syn{Principal}, \syn{Count} and \syn{CountByPrin}. See listing~\ref{lst:exampleconst} for the definition of \syn{constraint} and examples of how the three different kinds of constraints are used.
...
We started with the encoding of the agreement for Alice and Bob in chapter~\ref{chap:odrl0syntax} but we deferred the definition of the Coq constructs used to encode that agreement. All the definitions needed to encode the agreement for Alice and Bob have now been defined so we now show how that agreement looks like as encodings of \ac{ACCPL} constructs in Coq in the listing~\ref{lst:agreementAliceAndBob}. Recall the listing~\ref{lst:agreementpucella2006} for \syn{<agreement>} which used the syntax taken from~\cite{pucella2006}.
